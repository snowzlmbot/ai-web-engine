...
if ! curl -fsSL --retry 2 --connect-timeout 15 "$(resource_url "$RELEASE_BASE_URL/v$REMOTE_VERSION/ai-web-engine-android-arm64")" -o "$BIN_NEW" || ! chmod 755 "$BIN_NEW" || ! binary_valid "$BIN_NEW"; then
  rm -f "$BIN_NEW"
...
if ! binary_valid "$BINARY" || ! skills_valid "$SKILL_DIR" || ! script_valid "$START_SCRIPT" || ! script_valid "$STOP_SCRIPT" || ! config_valid "$CONFIG" || [ "$(wc -c < "$MASTER_KEY" | tr -d ' \n\t')" != "32" ]; then
  restore_old_payloads
  rm -f "$SKILL_NEW"
  rm -rf "$STAGE"
...
log "[DONE] Android 首次环境初始化完成：引擎、skills、启动脚本、停止脚本、配置、master.key 和目录均已就绪，版本 $REMOTE_VERSION"
exit 0
